...
# Overall1
```{r overall_continuous_donor1, fig.height=10, fig.width=14, echo=FALSE, message=FALSE, warning=FALSE}
...
r <- c('G. Mean peak AST levels (UI/L)', rep(NA, 6))
s <- c(paste0(unique(ast3$intervention), ' vs ', unique(ast3$comparison)), ast3_md$ma$k, 
...
overall1 <- rbind(a1, NA, NA,
                  ...
                  head2, NA, n,o, NA, p,q, NA, r,s, NA)
overall1 <- as.data.frame(overall1)

rownames(overall1) <- NULL

sub1 <- c(7,8,11,12,15,16, 19,20, 26, 29, 32) 
overall1[,1][sub1] <- paste("       ",overall1[,1][sub1])

sub2 <- c(6,10,14,18,25,28,31) 
...
plot1 <- forestplot(overall1,
             ...
             is.summary = c(T, rep(F,2), T, F, T, rep(F,3), T, rep(F,3), T, rep(F,3), T, rep(F,3), 
                            F, T, F, T, rep(F,2), T, rep(F,2), T, rep(F,2)),
             align = c('l', rep('c',4), 'l'),
             ci.vertices = TRUE,
             txt_gp = fpTxtGp(label =gpar (cex=0.9), 
                              ticks = gpar(cex = 0.9, fontface="bold"),
