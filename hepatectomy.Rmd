...
# Overall1
```{r overall_continuous_donor1, fig.height=6, fig.width=17, echo=FALSE, message=FALSE, warning=FALSE}
...
c <- c(paste0('A. ', surt1$intervention, ' vs ', surt1$comparison), surt1_md$ma$k, 
       sum(surt1$n1), sum(surt1$n2), 'NA', tsurt1$b[[4,6]], 
       'Suppl Fig #1')
d <- c(paste0('B. ', unique(surt2$intervention), ' vs ', unique(surt2$comparison)), surt2_md$ma$k, 
...
f <- c(paste0('C. ', unique(blood1$intervention), ' vs ', unique(blood1$comparison)), blood1_md$ma$k, 
       sum(blood1$n1), sum(blood1$n2), 'NA', tblood1$b[[4,6]], 
       'Suppl Fig #3')
g <- c(paste0('D. ', unique(blood2$intervention), ' vs ', unique(blood2$comparison)), blood2_md$ma$k, 
...
i <- c(paste0('E. ', unique(alt1$intervention), ' vs ', unique(alt1$comparison)), alt1_md$ma$k, 
       sum(alt1$n1), sum(alt1$n2), 
       ifelse(alt1_md$ma$k == 1, paste('NA'), paste0(formatC(alt1_md$ma$I2, format='f', digits=0),'%')),
       talt1$b[[4,6]], 
       'Suppl Fig #5')
j <- c(paste0('F. ', unique(alt2$intervention), ' vs ', unique(alt2$comparison)), alt2_md$ma$k, 
...
l <- c(paste0('G. ', unique(ast1$intervention), ' vs ', unique(ast1$comparison)), ast1_md$ma$k, 
       sum(ast1$n1), sum(ast1$n2), 
       ifelse(ast1_md$ma$k == 1, paste('NA'), paste0(formatC(ast1_md$ma$I2, format='f', digits=0),'%')),
       tast1$b[[4,6]], 
       'Suppl Fig #7')
m <- c(paste0('H. ', unique(ast2$intervention), ' vs ', unique(ast2$comparison)), ast2_md$ma$k, 
...
overall1 <- rbind(a1,b,c,d,e,f,g,h,i,j,k,l,m)
...
overall1 <- overall1 %>% add_row(.after = 1)
overall1 <- overall1 %>% add_row(.after = 6)

overall1 <- overall1 %>% add_row(.after = 10)
overall1 <- overall1 %>% add_row(.after = 14)
overall1 <- overall1 %>% add_row(.after = 18)


sub1 <- c(5,6,9,10,13,14,17,18) 
overall1[,1][sub1] <- paste("      ",overall1[,1][sub1])



toverall1 <- rbind(NA, NA, NA, NA, tsurt1$c[4,], tsurt2$c[9,],
                  NA, NA, tblood1$c[4,], tblood2$c[9,],
                  NA, NA, talt1$c[4,], talt2$c[8,],
                  NA, NA, tast1$c[4,], tast2$c[8,], NA)
...
forestplot(overall1,
             ...
             boxsize = 0.5,
             line.margin = 2,
             is.summary = c(T, rep(F,2), T, rep(F,3), T, rep(F,3), T, rep(F,3), T, rep(F,3)),
             align = c('l', rep('c',4), 'l'),
             ci.vertices = TRUE,
             txt_gp = fpTxtGp(label =gpar (cex=0.9), 
                              ticks = gpar(cex = 0.9, fontface="bold"),
                              summary = gpar(cex = 0.9),
                              xlab = gpar(cex=0.9)),
             xticks = c(-250, -150,-50,0,50,150, 250),
             xlog=FALSE,
             clip = c(0.2, 32),
             grid = c(-250, -150,-50,0,50,150, 250),
             lwd.xaxis = 1,
             lwd.ci = 2.2,
             lwd.zero = 2,
             graphwidth = unit(10,"cm"),
...
# Overall2
```{r overall_continuous_donor2, fig.height=6, fig.width=17, echo=FALSE, message=FALSE, warning=FALSE}

a2 <- c('Outcome', 'N of studies', 
       'Amr1 (n)', 'Arm2 (n)', 
       'I2', 
       paste0('Mean Difference (MD)','\n','95%CI'), 
       paste0('Figure with','\n', 'Detailed Information'))
...
o <- c(paste0('A. ', unique(bil1$intervention), ' vs ', unique(bil1$comparison)), bil1_md$ma$k, 
       sum(bil1$n1), sum(bil1$n2), 
       ifelse(bil1_md$ma$k == 1, paste0('NA'), paste0(formatC(bil1_md$ma$I2, format='f', digits=0),'%')),
       tbil1$b[[4,6]], 
       'Suppl Fig #9')
p <- c(paste0('B. ',unique(bil2$intervention), ' vs ', unique(bil2$comparison)), bil2_md$ma$k, 
...
r <- c(paste0('C. ', unique(pain2_fu1$pre$intervention), ' vs ', unique(pain2_fu1$pre$comparison), ' (',
...
s <- c(paste0('D. ', unique(pain2_fu2$pre$intervention), ' vs ', unique(pain2_fu2$pre$comparison), ' (',
       ...
       tpain2_fu2$b[[4,7]], 
       'Suppl Fig #11')
t <- c(paste0('E. ', unique(pain2_fu3$pre$intervention), ' vs ', unique(pain2_fu3$pre$comparison), ' (',
...
u <- c(paste0('F. ', unique(pain3_fu1$pre$intervention), ' vs ', unique(pain3_fu1$pre$comparison), ' (',
       ...
       tpain3_fu1$b[[8,6]], 
       'Suppl Fig #12')
v <- c(paste0('G. ', unique(pain3_fu2$pre$intervention), ' vs ', unique(pain3_fu2$pre$comparison), ' (',
       ...
       tpain3_fu2$b[[7,6]], 
       'Suppl Fig #13')
w <- c(paste0('H. ', unique(pain3_fu3$pre$intervention), ' vs ', unique(pain3_fu3$pre$comparison), ' (',
...
y <- c(paste0('I. ', unique(hospital1$intervention), ' vs ', unique(hospital1$comparison)), hospital1_md$ma$k, 
       ...
       thospital1$b[[4,6]], 
       'Suppl Fig #15')
z <- c(paste0('J. ', unique(hospital2$intervention), ' vs ', unique(hospital2$comparison)), hospital2_md$ma$k, 
...
overall2 <- rbind(a2,n,o,p,q,r,s,t,u,v,w,x,y,z)
overall2 <- as.data.frame(overall2)

rownames(overall2) <- NULL

overall2 <- overall2 %>% add_row(.after = 1)
overall2 <- overall2 %>% add_row(.after = 1)

overall2 <- overall2 %>% add_row(.after = 6)
overall2 <- overall2 %>% add_row(.after = 14)
overall2 <- overall2 %>% add_row(.after = 18)


sub1 <- c(5,6,9:14,17,18) 
overall2[,1][sub1] <- paste("      ",overall2[,1][sub1])



toverall2 <- rbind(NA, NA, NA, NA, tbil1$c[4,], tbil2$c[8,],
...
forestplot(overall2,
             ...
             boxsize = 0.5,
             line.margin = 2,
             is.summary = c(T, rep(F,2), T, rep(F,3), T, rep(F, 7), T, rep(F,3)),
             align = c('l', rep('c',4), 'l'),
             ci.vertices = TRUE,
             txt_gp = fpTxtGp(label =gpar (cex=0.9), 
                              ticks = gpar(cex = 0.9, fontface="bold"),
